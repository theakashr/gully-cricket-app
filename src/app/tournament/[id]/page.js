...
export default function TournamentPage({ params: paramsPromise }) {
  ...
  return (
    <div className="container mx-auto px-2 md:px-4 py-6 md:py-8 max-w-4xl pb-24 md:pb-8">
      <Link href="/" className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-900 font-bold text-sm mb-6 transition-all px-2">
        <ArrowLeft size={16} /> Back to Matches
      </Link>

      {/* Tournament Header */}
      <div className="glass-card rounded-3xl p-6 md:p-8 relative overflow-hidden mb-6 md:mb-8 mx-2 md:mx-0 border border-white shadow-sm">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 relative z-10 text-center md:text-left">
          <div className="w-20 h-20 bg-blue-50 rounded-2xl flex items-center justify-center border border-blue-100 shadow-sm flex-shrink-0">
            <Trophy size={40} className="text-blue-600 animate-pulse" />
          </div>
          <div>
            <span className={`inline-block text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full mb-2 shadow-sm ${tournament.status === 'active' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-slate-100 text-slate-500 border border-slate-200'}`}>
              {tournament.status}
            </span>
            <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tight leading-none">
              {tournament.name}
            </h1>
            <p className="text-slate-450 text-sm font-semibold mt-2.5">IPL Tournament Dashboard & Broadcasting Stats</p>
          </div>
        </div>
        <div className="absolute -right-10 -bottom-10 opacity-[0.02] pointer-events-none text-slate-900">
           <Trophy size={200} />
        </div>
      </div>
      ...
      {/* ========================================================
          ...
          ======================================================== */}
      <div className="flex bg-slate-100/90 border border-slate-200 p-1.5 rounded-2xl mb-6 mx-2 md:mx-0 overflow-x-auto scrollbar-hide shadow-inner">
        <button 
          onClick={() => setActiveTab('standings')}
          className={`flex-1 min-w-[90px] py-3 px-2 md:px-4 rounded-xl font-bold text-xs md:text-sm transition-all flex items-center justify-center gap-1 md:gap-2 ${activeTab === 'standings' ? 'bg-white text-emerald-700 shadow' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <List size={16} className="hidden md:block" /> Points Table
        </button>
        <button 
          onClick={() => setActiveTab('fixtures')}
          className={`flex-1 min-w-[90px] py-3 px-2 md:px-4 rounded-xl font-bold text-xs md:text-sm transition-all flex items-center justify-center gap-1 md:gap-2 ${activeTab === 'fixtures' ? 'bg-white text-emerald-700 shadow' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <Calendar size={16} className="hidden md:block" /> Fixtures & Results
        </button>
        <button 
          onClick={() => setActiveTab('stats')}
          className={`flex-1 min-w-[90px] py-3 px-2 md:px-4 rounded-xl font-bold text-xs md:text-sm transition-all flex items-center justify-center gap-1 md:gap-2 ${activeTab === 'stats' ? 'bg-white text-emerald-700 shadow' : 'text-slate-500 hover:text-slate-700'}`}
        >
          <BarChart3 size={16} className="hidden md:block" /> IPL Leaders
        </button>
      </div>
          ...
          <div className="space-y-6">
             ...
             {(liveMatch || recentMatch) && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                   {liveMatch && (
                      <Link href={`/match/${liveMatch.id}`} className="block relative group overflow-hidden rounded-3xl p-6 border border-red-100 bg-white hover:shadow-lg hover:-translate-y-1 transition-all shadow-sm">
                        ...
                        <div className="flex justify-between items-center mb-4">
                           ...
                           {(liveMatch.matchName || liveMatch.stage) && <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-2 py-0.5 rounded border border-slate-100">{liveMatch.matchName || liveMatch.stage}</span>}
                        </div>
                        ...
                        {/* Live Score Block */}
                        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex justify-between items-center relative overflow-hidden">
                           <div className="absolute right-0 bottom-0 opacity-[0.02] -mr-4 -mb-4">
                              <BarChart3 size={100} />
                           </div>
                           <div>
                   ...
                   {recentMatch && (
                      <Link href={`/match/${recentMatch.id}`} className="block relative group overflow-hidden rounded-3xl p-6 border border-slate-150 bg-white hover:shadow-lg hover:-translate-y-1 transition-all shadow-sm">
                        <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="mt-4 text-center">
                           <p className="text-[11px] font-black text-green-700 bg-green-50 inline-block px-3 py-1 rounded-full border border-green-150 uppercase tracking-wider">{recentMatch.result?.margin}</p>
                           {getMatchMVP(recentMatch) && (
                              <p className="text-[10px] text-slate-450 mt-2 uppercase tracking-widest">MVP: <span className="text-amber-600 font-bold">{getMatchMVP(recentMatch).name}</span></p>
                           )}
                        </div>
                      </Link>
                   )}
             ...
             <div className="grid grid-cols-2 gap-3 md:gap-6 mb-6">
                <div className="glass-card bg-white rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden border border-orange-200 shadow-sm hover:-translate-y-1 transition-all">
                   ...
                   <div className="relative z-10">
                     <p className="text-[9px] md:text-xs font-black text-orange-600 bg-orange-50 inline-block px-2.5 py-0.5 rounded uppercase tracking-widest mb-1 border border-orange-100">Orange Cap</p>
                ...
                <div className="glass-card bg-white rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden border border-purple-200 shadow-sm hover:-translate-y-1 transition-all">
                   <div className="absolute top-0 right-0 p-2 opacity-[0.08] text-purple-600">
                     <Medal size={64} />
                   </div>
                   <div className="relative z-10">
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody>
                      {pointsTable.length === 0 ? (
                        <tr>
                          <td colSpan="7" className="p-8 text-center text-slate-400 font-bold">No matches played yet.</td>
                        </tr>
                      ) : (
                        pointsTable.map((row, index) => {
                          ...
                          return (
                            <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                              ...
                              <td className="p-2 md:p-4 text-center font-bold text-slate-600 text-xs md:text-base">{row.played}</td>
                              ...
                              <td className="p-2 md:p-4 text-center font-bold text-slate-600 text-[10px] md:text-sm whitespace-nowrap">
          ...
          <div className="space-y-4">
             {matches.length === 0 ? (
               <div className="glass bg-white p-8 md:p-12 rounded-2xl text-center border border-slate-200/80 shadow-sm">
                 <Calendar size={40} className="mx-auto text-slate-350 mb-4" />
                 <h3 className="text-lg md:text-xl font-bold text-slate-405">No Fixtures Scheduled</h3>
               </div>
             ) : (
               matches.map(m => {
                           ...
                           <div className="mt-4 pt-3 border-t border-slate-100 text-center flex flex-col items-center gap-2">
                              ...
                              {mvp && (
                                 <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-yellow-50 border border-yellow-200 rounded-full shadow-sm">
                                    ...
                                    <span className="text-[10px] font-black text-slate-850">{mvp.name}</span>
                                 </div>
                              )}
                           </div>
                        )}
                    ...
                    {playerStats.orangeCap.map((p, i) => (
                      <li key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                        <div className="flex items-center gap-4">
                          <span className={`font-black text-lg w-6 text-center ${i === 0 ? 'text-orange-500 text-2xl' : 'text-slate-350'}`}>{i + 1}</span>
                          <div>
                            <p className="font-bold text-slate-800 text-sm md:text-base">{p.name}</p>
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">{getTeamDetails(p.teamId).shortName}</p>
                          </div>
                        </div>
                ...
                {statsTab === 'purple' && (
                  ...
                  <ul className="divide-y divide-slate-100">
                    {playerStats.purpleCap.map((p, i) => (
                      <li key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                        <div className="flex items-center gap-4">
                          <span className={`font-black text-lg w-6 text-center ${i === 0 ? 'text-purple-500 text-2xl' : 'text-slate-350'}`}>{i + 1}</span>
                          <div>
                            <p className="font-bold text-slate-800 text-sm md:text-base">{p.name}</p>
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">{getTeamDetails(p.teamId).shortName}</p>
                          </div>
                ...
                {statsTab === 'mvp' && (
                  ...
                  <ul className="divide-y divide-slate-100">
                    {playerStats.mvp.map((p, i) => (
                      <li key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                        <div className="flex items-center gap-4">
                          <span className={`font-black text-lg w-6 text-center ${i === 0 ? 'text-yellow-600 text-2xl' : 'text-slate-350'}`}>{i + 1}</span>
                          <div>
                            <p className="font-bold text-slate-800 text-sm md:text-base">{p.name}</p>
                            <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">{getTeamDetails(p.teamId).shortName}</p>
                          </div>
                        </div>
                        <div className="text-base md:text-lg font-black text-yellow-600 bg-yellow-50 border border-yellow-100 px-3 py-1 rounded-lg shadow-sm">{p.mvpPoints} <span className="text-[9px] text-yellow-650 uppercase tracking-widest font-black">pts</span></div>
                      </li>
                    ))}
                  </ul>
                )}
