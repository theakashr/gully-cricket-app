...
import { Trophy, Calendar, List, Shield, BarChart3, Medal, Star, Play, Award, ChevronRight, TrendingUp, ArrowUpRight } from 'lucide-react';
...
export default function PublicTournamentsPage() {
  ...
  // Fetch Tournaments List & Teams
  useEffect(() => {
    const fetchTourneysAndTeams = async () => {
      ...
      onValue(tournamentsRef, (snapshot) => {
        ...
        if (data) {
          ...
          const sortedList = tList.reverse(); // Newest first
          setTournaments(sortedList);
          
          // Default to the first active or newest tournament
          if (sortedList.length > 0) {
            setSelectedTourneyId(sortedList[0].id);
          }
        } else {
          setTournaments([]);
        }
    ...
    fetchTourneysAndTeams();
  }, []);

  // Fetch selected tournament data (matches, standings, caps)
  useEffect(() => {
    ...
    const unsubscribe = onValue(matchesRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        const tMatches = allMatches.filter(m => m.tournamentId === selectedTourneyId);
        
        tMatches.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
        setMatches(tMatches);
        
        calculatePointsTable(tMatches, teams);
        calculatePlayerStats(tMatches);
      } else {
        setMatches([]);
        setPointsTable([]);
        setPlayerStats({ orangeCap: [], purpleCap: [], mvp: [] });
      }
      setLoadingData(false);
    });

    return () => unsubscribe();
  }, [selectedTourneyId, tournaments, teams]);
  ...
  return (
    <div className="container mx-auto px-3 sm:px-4 py-8 max-w-4xl pb-24 md:pb-12">
      
      {/* 📋 Section 1: Tournament Selector */}
      {loadingList ? (
        <div className="flex justify-center py-12">
          <div className="w-8 h-8 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin"></div>
        </div>
      ) : tournaments.length === 0 ? (
        <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-sm mb-8">
          <Trophy size={64} className="mx-auto text-slate-305 mb-4 animate-bounce-slow" />
          <h2 className="text-2xl font-black text-slate-800">No Tournaments Found</h2>
          <p className="text-slate-500 font-semibold mt-2">Check back later when tournaments are pre-configured!</p>
        </div>
      ) : (
        <div className="mb-8">
          <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2.5 block pl-1">
             Select Active League / Tournament
          </label>
          <div className="flex bg-slate-100 p-1.5 rounded-2xl overflow-x-auto scrollbar-hide border border-slate-200/80 shadow-inner gap-1">
            {tournaments.map(t => (
              <button
                key={t.id}
                onClick={() => setSelectedTourneyId(t.id)}
                className={`px-5 py-3 rounded-xl font-black text-xs md:text-sm whitespace-nowrap transition-all uppercase flex items-center gap-2 ${
                  selectedTourneyId === t.id
                    ? 'bg-white text-blue-700 shadow-md scale-102'
                    : 'text-slate-500 hover:text-slate-800 hover:bg-white/40'
                }`}
              >
                <Trophy size={16} className={selectedTourneyId === t.id ? 'text-blue-600' : 'text-slate-400'} />
                {t.name}
                {t.status === 'active' && (
      ...
      {selectedTournament && (
        ...
        >
          ...
          <div className="glass-card rounded-3xl p-6 md:p-8 relative overflow-hidden border border-white shadow-sm">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-6 relative z-10 text-center md:text-left">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-500 text-sm font-semibold mt-2.5">Real-time Brackets, Standings & Win Probabilities</p>
              </div>
            </div>
          </div>
          ...
          {/* ========================================================
              ...
              ======================================================== */}
          <div className="glass-card rounded-3xl p-6 relative overflow-hidden border border-slate-200/80 shadow-md bg-white">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-100 pb-5 mb-5">
              ...
              <div className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200/40">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl mb-4 border border-slate-200/50 max-w-md mx-auto shadow-inner text-xs">
              <button 
                ...
                className={`flex-1 py-2 font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${analyticsTab === 'worm' ? 'bg-white text-slate-800 shadow' : 'text-slate-500 hover:text-slate-855'}`}
              >
                📈 Worm Chart
              </button>
              <button 
                onClick={() => setAnalyticsTab('manhattan')}
                className={`flex-1 py-2 font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${analyticsTab === 'manhattan' ? 'bg-white text-slate-800 shadow' : 'text-slate-500 hover:text-slate-855'}`}
              >
                📊 Manhattan
              </button>
              <button 
                onClick={() => setAnalyticsTab('projection')}
                className={`flex-1 py-2 font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${analyticsTab === 'projection' ? 'bg-white text-slate-800 shadow' : 'text-slate-500 hover:text-slate-855'}`}
              >
                🎯 Projections
              </button>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-150 rounded-2xl p-4 min-h-[220px] flex items-center justify-center relative shadow-inner overflow-hidden">
               ...
               {analyticsTab === 'worm' && (
                  <div className="w-full flex flex-col items-center">
                     ...
                     {/* Legend */}
                     <div className="flex gap-4 mt-3 justify-center text-[10px] font-black uppercase tracking-wider text-slate-650">
          ...
          {/* ========================================================
              📋 Section 3: Points Table, Fixtures, Caps Tabs
              ======================================================== */}
          {loadingData ? (
            <div className="flex justify-center py-12">
              <div className="w-8 h-8 border-4 border-blue-500/30 border-t-blue-500 rounded-full animate-spin"></div>
            ...
            <div className="space-y-6">
              <div className="flex bg-slate-150 p-1.5 rounded-2xl overflow-x-auto scrollbar-hide shadow-inner border border-slate-200">
                <button 
                  onClick={() => setActiveTab('standings')}
                  className={`flex-1 min-w-[95px] py-3.5 px-4 rounded-xl font-black text-xs md:text-sm transition-all uppercase flex items-center justify-center gap-2 ${activeTab === 'standings' ? 'bg-white text-blue-700 shadow' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  <List size={16} /> Points Table
                </button>
                <button 
                  onClick={() => setActiveTab('fixtures')}
                  className={`flex-1 min-w-[95px] py-3.5 px-4 rounded-xl font-black text-xs md:text-sm transition-all uppercase flex items-center justify-center gap-2 ${activeTab === 'fixtures' ? 'bg-white text-blue-700 shadow' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  <Calendar size={16} /> Fixtures
                </button>
                <button 
                  onClick={() => setActiveTab('stats')}
                  className={`flex-1 min-w-[95px] py-3.5 px-4 rounded-xl font-black text-xs md:text-sm transition-all uppercase flex items-center justify-center gap-2 ${activeTab === 'stats' ? 'bg-white text-blue-700 shadow' : 'text-slate-500 hover:text-slate-800'}`}
                >
                  <BarChart3 size={16} /> Cap Leaders
                </button>
              </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                             ...
                             {recentMatch && (
                                <Link href={`/match/${recentMatch.id}`} className="block relative group overflow-hidden rounded-3xl p-6 border border-slate-205 bg-white hover:shadow-lg hover:-translate-y-1 transition-all shadow-sm">
                                  <div className="flex justify-between items-center mb-4">
                                     <span className="text-[10px] font-bold text-slate-555 bg-slate-100 px-3 py-1 rounded-full border border-slate-200/50">Recent Match</span>
                                  ...
                                  <div className="space-y-4">
                                     ...
                                     <div className="flex justify-between items-center bg-slate-50 border border-slate-150 rounded-xl p-3">
                                        <div className="flex items-center gap-3">
                                          <div className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center overflow-hidden border border-slate-200">
                                            {getTeamDetails(recentMatch.teamB).logoUrl ? <img src={getTeamDetails(recentMatch.teamB).logoUrl} alt="Logo" className="w-full h-full object-cover" /> : <Shield size={14} className="text-slate-405"/>}
                                          </div>
                                  ...
                                  <div className="mt-4 text-center">
                                     <p className="text-[11px] font-black text-green-755 bg-green-50 inline-block px-3 py-1 rounded-full border border-green-150 uppercase tracking-wider">{recentMatch.result?.margin}</p>
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          <div className="glass-card bg-white rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden border border-orange-200 shadow-sm hover:-translate-y-1 transition-all">
                             ...
                             <div className="relative z-10">
                               ...
                               <p className="text-sm md:text-xl font-bold text-slate-850 truncate mt-1">{orangeLeader ? orangeLeader.name : 'N/A'}</p>
                          ...
                          <div className="glass-card bg-white rounded-2xl p-4 flex flex-col justify-between relative overflow-hidden border border-purple-200 shadow-sm hover:-translate-y-1 transition-all">
                             ...
                             <div className="relative z-10">
                               ...
                               <p className="text-sm md:text-xl font-bold text-slate-850 truncate mt-1">{purpleLeader ? purpleLeader.name : 'N/A'}</p>
                            ...
                            <table className="w-full text-left border-collapse">
                              <thead>
                                <tr className="border-b border-slate-200 bg-slate-50/80">
                                  ...
                                  <th className="p-3 md:p-4 text-[10px] md:text-xs font-black text-blue-750 uppercase tracking-widest text-center bg-blue-50/50">PTS</th>
                                </tr>
                              </thead>
                              <tbody>
                                {pointsTable.length === 0 ? (
                                  <tr>
                                    <td colSpan="7" className="p-8 text-center text-slate-400 font-bold">No league matches played yet.</td>
                                  </tr>
                                ) : (
                                  pointsTable.map((row, index) => {
                                    ...
                                    return (
                                      <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="p-2 md:p-4 text-center font-bold text-slate-655 text-[10px] md:text-sm whitespace-nowrap">
                    ...
                    <div className="space-y-4">
                       {matches.length === 0 ? (
                          <div className="glass bg-white p-8 md:p-12 rounded-2xl text-center border border-slate-200/80 shadow-sm">
                            <Calendar size={40} className="mx-auto text-slate-400 mb-4 animate-pulse" />
                                ...
                                <div className="glass-card bg-white rounded-2xl p-4 md:p-6 hover:shadow hover:-translate-y-1 transition-all relative overflow-hidden group border border-slate-200/80">
                                   ...
                                   <div className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-150">
                                      <div className="flex flex-col md:flex-row items-center gap-2 md:gap-3 w-1/3 md:w-auto">
                                        ...
                                        <span className="font-black text-slate-805 text-sm md:text-xl text-center">{tA.shortName}</span>
                                      </div>
                                      ...
                                      <div className="flex flex-col-reverse md:flex-row items-center gap-2 md:gap-3 w-1/3 md:w-auto">
                                        <span className="font-black text-slate-805 text-sm md:text-xl text-center">{tB.shortName}</span>
                                        <div className="w-10 h-10 md:w-14 md:h-14 rounded-full bg-white shadow-sm flex flex-shrink-0 items-center justify-center overflow-hidden border border-slate-200">
                  ...
                  {activeTab === 'stats' && (
                    <div className="space-y-4">
                       ...
                       <div className="glass-card bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm">
                          {statsTab === 'orange' && (
                            playerStats.orangeCap.length === 0 ? <p className="p-8 text-center text-slate-400 font-bold">No statistics calculated yet for this tournament.</p> :
                              ...
                              {playerStats.orangeCap.map((p, i) => (
                                <li key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                                  <div className="flex items-center gap-4">
                                    <span className={`font-black text-lg w-6 text-center ${i === 0 ? 'text-orange-500 text-2xl' : 'text-slate-300'}`}>{i + 1}</span>
                                    <div>
                                  ...
                                  <div className="text-base md:text-lg font-black text-orange-655 bg-orange-50 border border-orange-100 px-3 py-1 rounded-lg shadow-sm">{p.runs} runs</div>
                                </li>
                              ))}
                            </ul>
                          )}
                          {statsTab === 'purple' && (
                            playerStats.purpleCap.length === 0 ? <p className="p-8 text-center text-slate-400 font-bold">No statistics calculated yet for this tournament.</p> :
                            <ul className="divide-y divide-slate-100">
                              {playerStats.purpleCap.map((p, i) => (
                                <li key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                                  <div className="flex items-center gap-4">
                                    <span className={`font-black text-lg w-6 text-center ${i === 0 ? 'text-purple-500 text-2xl' : 'text-slate-300'}`}>{i + 1}</span>
                                    <div>
                                      <p className="font-bold text-slate-800 text-sm md:text-base">{p.name}</p>
                                      <p className="text-[10px] text-slate-450 font-black uppercase tracking-wider">{getTeamDetails(p.teamId).shortName}</p>
                                    </div>
                                  </div>
                                  <div className="text-base md:text-lg font-black text-purple-600 bg-purple-50 border border-purple-100 px-3 py-1 rounded-lg shadow-sm">{p.wickets} wkts</div>
                                </li>
                              ))}
                            </ul>
                          )}
                          {statsTab === 'mvp' && (
                            playerStats.mvp.length === 0 ? <p className="p-8 text-center text-slate-400 font-bold">No statistics calculated yet for this tournament.</p> :
                            <ul className="divide-y divide-slate-100">
                              {playerStats.mvp.map((p, i) => (
                                <li key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50/50 transition-colors">
                                  <div className="flex items-center gap-4">
                                    <span className={`font-black text-lg w-6 text-center ${i === 0 ? 'text-yellow-600 text-2xl' : 'text-slate-300'}`}>{i + 1}</span>
                                    <div>
                                      <p className="font-bold text-slate-800 text-sm md:text-base">{p.name}</p>
                                      <p className="text-[10px] text-slate-450 font-black uppercase tracking-wider">{getTeamDetails(p.teamId).shortName}</p>
                                    </div>
